...
class MotorsBus(abc.ABC):
    ...
    def set_zero_homings(self, motors: NameOrID | list[NameOrID] | None = None) -> dict[NameOrID, Value]:
        """Set the current position as zero by updating the homing offset."""
        if motors is None:
            motors_list = list(self.motors)
        elif isinstance(motors, (str, int)):
            motors_list = [motors]
        elif isinstance(motors, list):
            motors_list = motors.copy()
        else:
            raise TypeError(motors)

        motors_list = [
            self._id_to_name(motor) if isinstance(motor, int) else motor for motor in motors_list
        ]

        positions = self.sync_read("Present_Position", motors_list, normalize=False)
        offsets = self.sync_read("Homing_Offset", motors_list, normalize=False)

        new_offsets = {
            motor: offsets[motor] - positions[motor] for motor in motors_list
        }

        for motor, offset in new_offsets.items():
            self.write("Homing_Offset", motor, offset, normalize=False)
            if motor in self.calibration:
                self.calibration[motor].homing_offset = offset

        after_positions = self.sync_read("Present_Position", motors_list, normalize=False)
        after_offsets = self.sync_read("Homing_Offset", motors_list, normalize=False)
        print(
            f"{self.__class__.__name__} zero homings updated: "
            f"before_pos={positions} after_pos={after_positions} "
            f"before_off={offsets} after_off={after_offsets}"
        )
